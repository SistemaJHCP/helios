$(document).ready(function() {
    ...
    $('#cuadrilla').on('click', function(){
        ...
        if( $('#cuadrilla').prop('checked') ) {
            ...
            $('#cua_finish').prop('disabled', false);
        } else {
    ...
    $("#nombre_politica").on('blur', function(){
        if ($("#nombre_politica").val().length > 3) {
            ...
            .done(function(comp) {
                if (comp[0].id != $("#id_1").val()) {
                    alert("el nombre esta siendo usado, por favor use otro");
                }
            })
            .fail( function(){
